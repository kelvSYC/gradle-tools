...
class BitSetBitShiftSpec : FunSpec() {
    init {
        test("leftShift") {
            checkAll(Arb.int(), Arb.int(0 ..< Int.SIZE_BITS)) { value, bitCount ->
                ...
                val result = BitSetBitShift(Int.SIZE_BITS).leftShift(bitset, bitCount)
        ...
        test("rightShift") {
            checkAll(Arb.int(), Arb.int(0 ..< Int.SIZE_BITS)) { value, bitCount ->
                ...
                val result = BitSetBitShift(Int.SIZE_BITS).rightShift(bitset, bitCount)
                val rebuilt = result.toByteArray().foldIndexed(0) { index, acc, b ->
                    acc or ((b.toInt() and 0xFF) shl (index * Byte.SIZE_BITS))
                }
        ...
        test("arithmeticRightShift") {
            checkAll(Arb.int(), Arb.int(0 ..< Int.SIZE_BITS)) { value, bitCount ->
                ...
                val result = BitSetBitShift(Int.SIZE_BITS).arithmeticRightShift(bitset, bitCount)
                val rebuilt = result.toByteArray().foldIndexed(0) { index, acc, b ->
                    acc or ((b.toInt() and 0xFF) shl (index * Byte.SIZE_BITS))
                }
