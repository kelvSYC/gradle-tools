...
class BitSetBitRotateSpec : FunSpec() {
    init {
        test("rotateLeft") {
            checkAll(Arb.int(), Arb.int(0 ..< Int.SIZE_BITS)) { value, bitCount ->
                ...
                val result = BitSetBitRotate(Int.SIZE_BITS).rotateLeft(bitset, bitCount)
        ...
        test("rotateRight") {
            checkAll(Arb.int(), Arb.int(0 ..< Int.SIZE_BITS)) { value, bitCount ->
                ...
                val result = BitSetBitRotate(Int.SIZE_BITS).rotateRight(bitset, bitCount)
                val rebuilt = result.toByteArray().foldIndexed(0) { index, acc, b ->
                    acc or ((b.toInt() and 0xFF) shl (index * Byte.SIZE_BITS))
                }
