 ...
 */
abstract class AbstractDPD64Bits<T>(bits: Long) : AbstractDecimal64Bits<T>(bits) {
    ...
    override val significand: Long? by lazy {
        when (discriminator) {
            Discriminator.LOW, Discriminator.HIGH -> (leadingDigit!!.toLong() shl CONTINUATION_WIDTH) or continuation
            else -> null
        }
    }
