...
class GetGitRemoteArchiveSpec : FunSpec() {
    init {
        context("GetGitRemoteArchive - property configuration") {
            test("remoteUrl can be set and retrieved") {
                val project = ProjectBuilder.builder().build()
                val task = project.tasks.register<GetGitRemoteArchive>("myTask").get()
            ...
            test("outputFile can be set and retrieved") {
                val project = ProjectBuilder.builder().build()
                val tempFile = Files.createTempFile("archive", ".zip").toFile()
                val task = project.tasks.register<GetGitRemoteArchive>("myTask").get()

                task.outputFile.set(tempFile)

                task.outputFile.get().asFile shouldBe tempFile
        ...
        context("GetGitRemoteArchive - plugin convention") {
            ...
            test("explicit gitCommand overrides the convention set by the plugin") {
                ...
                val task = project.tasks.register<GetGitRemoteArchive>("myTask") {
