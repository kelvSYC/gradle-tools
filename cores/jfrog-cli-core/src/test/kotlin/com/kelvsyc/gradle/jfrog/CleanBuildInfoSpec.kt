...
class CleanBuildInfoSpec : FunSpec() {
    init {
        context("CleanBuildInfo - property configuration") {
            test("buildName can be set and retrieved") {
                val project = ProjectBuilder.builder().build()
                val task = project.tasks.register<CleanBuildInfo>("myTask").get()
        ...
        context("CleanBuildInfo - plugin convention") {
            ...
            test("plugin applies and wires jfCommand convention without error") {
                ...
                project.tasks.register<CleanBuildInfo>("myTask").get()
            }
            ...
            test("explicit jfCommand overrides the convention set by the plugin") {
                ...
                val task = project.tasks.register<CleanBuildInfo>("myTask") {
