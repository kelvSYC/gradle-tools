...
class AddGitInfoToBuildActionSpec : FunSpec() {
    ...
    init {
        context("AddGitInfoToBuildAction - args structure") {
            test("basic args: rt, build-add-git, build name, build number") {
                val project = ProjectBuilder.builder().build()
                val execSpec = mockk<ExecSpec>(relaxed = true)
                val capturedArgs = captureArgs(execSpec)
                val execOps = mockExecOps(execSpec)

                val params = project.objects.newInstance<AddGitInfoToBuildAction.Parameters>()
                params.jfCommand.set("jf")
                params.buildName.set("my-build")
                params.buildNumber.set("42")

                object : AddGitInfoToBuildAction(execOps, project.providers) {
                    override fun getParameters() = params
                }.execute()

                capturedArgs shouldBe listOf("rt", "build-add-git", "my-build", "42")
            }
